    ...
    <div class="admin-container">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card new">
                <div class="stat-title">⚡ New / Uncontacted</div>
            ...
            <div class="stat-card in-progress">
                <div class="stat-title">🔧 Technician In Progress</div>
                <div class="stat-value"><?= $stats['in_progress'] ?></div>
            </div>
            <div class="stat-card completed">
                <div class="stat-title">✅ Completed / Serviced</div>
                    ...
                    <table class="leads-table">
                        ...
                        <tbody>
                            <?php foreach ($leads as $lead): ?>
                                ...
                                $createdTime = !empty($lead['created_at']) ? date('d M, h:i A', strtotime($lead['created_at'])) : 'N/A';
                                $badgeClass = 'badge-' . str_replace(' ', '', $lead['status'] ?? 'New');
                                ?>
                                <tr>
                                    <td>
                                        <strong style="color:#0788C9; font-size:12px;"><?= htmlspecialchars($lead['id']) ?></strong><br>
                                        <span style="color:#64748b; font-size:11px;"><?= $createdTime ?></span>
                                    </td>
                                    <td>
                                        <strong><?= htmlspecialchars($lead['name']) ?></strong><br>
                                        <?php if (!empty($lead['email'])): ?>
                                            <span style="font-size:11px; color:#64748b;"><?= htmlspecialchars($lead['email']) ?></span>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <div style="display:flex; flex-direction:column; gap:4px;">
                                            <a href="tel:<?= $cleanPhone ?>" class="btn-call" title="Call Customer">
                                                📞 <?= htmlspecialchars($lead['phone']) ?>
                                            </a>
                                            <a href="https://wa.me/91<?= $cleanPhone ?>?text=Hello%20<?= urlencode($lead['name']) ?>%2C%20this%20is%20FridgeFix%20Hyderabad%20regarding%20your%20service%20booking%20(<?= urlencode($lead['id']) ?>)." 
                                               target="_blank" class="btn-wa" title="Open WhatsApp Chat">
                                                💬 WhatsApp
                                            </a>
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        <span style="color:#0f172a; font-weight:600;"><?= htmlspecialchars($lead['location']) ?></span><br>
                                        <?php if (!empty($lead['preferred_date'])): ?>
                                            <span style="font-size:11px; color:#64748b;">Pref: <?= htmlspecialchars($lead['preferred_date']) ?></span>
                                        <?php endif; ?>
                                    </td>
                                    <td style="max-width:220px;">
                                        <div style="font-size:12px; color:#334155; line-height:1.4;">
                                            <?= htmlspecialchars($lead['problem'] ?? 'General Inspection') ?>
                                        </div>
                                        <?php if (!empty($lead['message'])): ?>
                                            <div style="font-size:11px; color:#64748b; margin-top:2px; font-style:italic;">
                                                Note: <?= htmlspecialchars($lead['message']) ?>
                                            </div>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <form method="POST" action="index.php" style="display:flex; flex-direction:column; gap:6px;">
                                    ...
                                    <td>
                                        <form method="POST" action="index.php" onsubmit="return confirm('Are you sure you want to permanently delete lead <?= htmlspecialchars($lead['id']) ?>?');">
                                            <input type="hidden" name="action" value="delete_lead">
                                            <input type="hidden" name="lead_id" value="<?= htmlspecialchars($lead['id']) ?>">
                                            <button type="submit" style="background:none; border:none; color:#ef4444; font-size:11px; cursor:pointer; padding:4px 8px; border-radius:4px;" title="Delete Lead">
                                                🗑️ Delete
                                            </button>
                                        </form>
                                    </td>
